...
from functions import get_pair_factors_of_number, get_nearest_pairs, smile_to_hot_reduced
...
class OneHotRNN(nn.Module):
    ...
    def __init__(self,
        ...
        # vocabulary
        self.vocabulary = vocabulary
        pairsx, _ = get_nearest_pairs(len(self.vocabulary))
        # reduction
        if reduction_type is not None:
            self.reduction_type = reduction_type
        if self.reduction_type == 'reduction1' or self.reduction_type == 'reduction2':
            self.vocabulary_size = pairsx[0]
            print('REDUCED VOCABULARY SIZE:', self.vocabulary_size )
        else:
            self.vocabulary_size = len(self.vocabulary)
        ...
        linear_dim1 = hidden_size * 2 if bidirectional else hidden_size
        if self.reduction_type == 'reduction1' or self.reduction_type == 'reduction2':
            self.decoder = nn.Linear(linear_dim1, self.vocabulary_size * pairsx[1])
        else:
            self.decoder = nn.Linear(linear_dim1, self.vocabulary_size)
        # self.relu = nn.ReLU()
        # loss function (ignoring padding)
    ...
    def forward(self, input, hidden):
        ...
        """
        # one-hot embed
        pairs, _ = get_nearest_pairs(len(self.vocabulary.dictionary))
        # print(self.reduction_type)
        # print('unprocessed input\t', input.shape,input)
        if self.reduction_type == 'reduction1':
            if torch.cuda.is_available():
                inputs = smile_to_hot_reduced(input, pairs, 'reduction1').float().cuda()
            else:
                inputs = smile_to_hot_reduced(input, pairs, 'reduction1').float()
        elif self.reduction_type == 'reduction2':
            if torch.cuda.is_available():
                inputs = smile_to_hot_reduced(input, pairs, 'reduction2').float().cuda()
            else:
                inputs = smile_to_hot_reduced(input, pairs, 'reduction2').float()
        else:
            if torch.cuda.is_available():
                # inputs = F.one_hot(input, self.vocabulary_size).float().cuda()
                inputs = smile_to_hot_reduced(input, self.vocabulary_size, 'reduction0').float().cuda()
            else:
                # inputs = F.one_hot(input, self.vocabulary_size).float()
                inputs = smile_to_hot_reduced(input, self.vocabulary_size, 'reduction0').float()
