...
from functions import get_pair_factors_of_number, get_nearest_pairs
...
class SmilesDataset(Dataset):
    ...
    def __init__(self, smiles=None, smiles_file=None, vocab_file=None, reduction_type=None,
                 training_split=0.9):
        """
        ...
        # create vocabulary
        if vocab_file:
            self.vocabulary = Vocabulary(vocab_file=vocab_file, reduction_type=self.reduction_type)
        else:
            self.vocabulary = Vocabulary(smiles=self.smiles, reduction_type=self.reduction_type)
...
class SelfiesDataset(Dataset):
    ...
    def __init__(self, selfies=None, selfies_file=None, vocab_file=None, reduction_type = None,
                 training_split=0.9):
        """
        ...
        if vocab_file:
            ...
            self.vocabulary = SelfiesVocabulary(selfies=self.selfies, reduction_type=self.reduction_type)
...
class Vocabulary(object):
    ...
    def __init__(self, smiles=None, smiles_file=None, vocab_file=None, reduction_type=None):
        ...
        if self.reduction_type != 'reduction0':
            _, pads_to_add = get_nearest_pairs(len(self.characters))
            if pads_to_add > 0:
                for i in range(pads_to_add):
                    newpad = '<PAD' + str(i) + '>'
                    self.characters.append(newpad)
...
class SelfiesVocabulary(object):
    ...
    def __init__(self, selfies=None, selfies_file=None, vocab_file=None, reduction_type=None):
        ...
        # add empty classes if necessary to reduced OHE
        if self.reduction_type != 'reduction0':
            _, pads_to_add = get_nearest_pairs(len(self.characters))
            if pads_to_add > 0:
                for i in range(pads_to_add):
