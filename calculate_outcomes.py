...
# os.chdir(python_dir)
try:
    path = '/scratch/s.2132253/ldgm'
    git_dir = os.path.expanduser(path)
    python_dir = git_dir + "/python"
    os.chdir(python_dir)
except:
    path = '~/Documents/Projects/ldgm'
    git_dir = os.path.expanduser(path)
    python_dir = git_dir + "/python"
    os.chdir(python_dir)
...
parser.add_argument('--original_file', type=str, help='file containing training SMILES')
...
parser.add_argument('--minimal', dest='minimal', help='calculate only % valid, % novel, and % uniques', action='store_true')
parser.add_argument('--selfies', dest='selfies', help='calculate outcomes for molecules in SELFIES format', action='store_true')
parser.add_argument('--deepsmiles', dest='deepsmiles', help='calculate outcomes for molecules in DeepSMILES format', action='store_true')
parser.add_argument('--sampled_files', type=str, nargs='*', help='file(s) containing sampled SMILES')
